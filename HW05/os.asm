...
; the TRAP vector table
	...
	JMP BAD_TRAP	; x07
	JMP BAD_TRAP	; x08
	JMP TRAP_SET_TIMER	; x09
	JMP TRAP_CHECK_TIMER	; x0A
	JMP TRAP_PUTS		; x0B
	JMP BAD_TRAP	; x0C
	JMP BAD_TRAP	; x0D
	JMP BAD_TRAP	; x0E
...
;;;  YOUR CODE HERE
	CMPIU R0, #0			; Checking bounds of rows
	BRn END_HLINE

	CMPIU R0, #123
	BRp END_HLINE

	CMPIU R1, #0			; Checking bounds of column
	BRn END_HLINE

	ADD R2, R2, R1			; Column Address 2

	CMPIU R2, #127			; Comparing R2 with 127
...
;;; TRAP_CHECK_TIMER - get the Timer Status Register
;;; Input
;;;   none
;;; Output
;;;   r0 - current value of TSR
...
;;;  YOUR CODE HERE
	LC R4, OS_ADSR_ADDR		; Load ADSR into R1
	LDR R1, R4, #0
	BRzp TRAP_PUTS			; Keep looping while MSB is zero/positive

	LDR R3, R0, #0			; Get character from R0
	BRnz END				; If character is negative or zero, RTI
	
	LC R4, OS_ADDR_ADDR		; Put ADDR into R4
	STR R3, R4, #0			; Otherwise, load val of character into R4
	
	ADD R0, R0, #1			; Increment address to next character
	JMP TRAP_PUTS
